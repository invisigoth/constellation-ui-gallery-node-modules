import defaultTranslation from './default';
export type TranslationPack = Readonly<typeof defaultTranslation>;
export type Translation = Partial<typeof defaultTranslation>;
declare const create: (translations?: Translation) => Readonly<TranslationPack>;
export default create;
export declare const DefaultTranslation: Readonly<Readonly<{
    yes: string;
    no: string;
    filters: string;
    actions: string;
    status: string;
    additional_info: string;
    search_placeholder_default: string;
    select_placeholder_default: string;
    description: string;
    all: string;
    default: string;
    app_default: string;
    sitewide: string;
    opens_in_a_new_tab: string;
    summary: string;
    utilities: string;
    search: string;
    menu: string;
    new: string;
    info: string;
    shortcuts: string;
    remove: string;
    follow: string;
    unfollow: string;
    add: string;
    cancel: string;
    try_again: string;
    update: string;
    submit: string;
    apply: string;
    select: string;
    edit: string;
    preview: string;
    decline: string;
    delete: string;
    close: string;
    activate: string;
    expand: string;
    collapse: string;
    accept: string;
    clear: string;
    use: string;
    undo: string;
    redo: string;
    filter: string;
    sort: string;
    group: string;
    go: string;
    configure: string;
    dismiss: string;
    view_all: string;
    view_less: string;
    show_more: string;
    show_less: string;
    show_all: string;
    clear_all: string;
    link_open_in_tab_text: string;
    edit_details: string;
    add_emoji: string;
    create_new: string;
    submit_and_open: string;
    add_condition: string;
    open_noun: string;
    close_noun: string;
    view_all_noun: string;
    view_less_noun: string;
    expand_noun: string;
    add_noun: string;
    collapse_noun: string;
    checked_noun: string;
    selected_noun: string;
    unselected_noun: string;
    use_input_value: string;
    preview_of_noun: string;
    go_to_noun: string;
    insert_noun: string;
    delete_noun: string;
    actions_for: string;
    edited: string;
    done: string;
    uploading: string;
    loading: string;
    loaded: string;
    submitting: string;
    no_items: string;
    unknown_error: string;
    image_load_error: string;
    selected: string;
    selected_count: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    completed: string;
    current: string;
    previous: string;
    not_started: string;
    empty_value: string;
    no_value: string;
    delivered: string;
    sent: string;
    active: string;
    read_only: string;
    information: string;
    error: string;
    warning: string;
    success: string;
    day_placeholder: string;
    month_placeholder: string;
    year_placeholder: string;
    hour_placeholder: string;
    minute_placeholder: string;
    second_placeholder: string;
    meridiem_value_am: string;
    meridiem_value_pm: string;
    week_placeholder: string;
    seconds_text: string;
    minutes_text: string;
    hours_text: string;
    days_text: string;
    time_text: string;
    time_format_info: string;
    date_quarter_q1: string;
    date_quarter_q2: string;
    date_quarter_q3: string;
    date_quarter_q4: string;
    time_unit: string;
    attachments_count: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    results_count: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    recent_results_count: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    new_emails_count: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    pagination_page_of: string;
    pagination_next: string;
    pagination_prev: string;
    range_from_datetime: string;
    range_to_datetime: string;
    range_from_numeric: string;
    range_to_numeric: string;
    x_of_y: string;
    preview_link_instruction: string;
    polite_announcements: string;
    assertive_announcements: string;
    go_to_main_content: string;
    main_content: string;
    app_shell_create: string;
    app_shell_open_nav: string;
    app_shell_main_navigation_label: string;
    shortcut_NextRegion: string;
    shortcut_PrevRegion: string;
    combobox_open_close: string;
    combobox_search_instructions: string;
    combobox_open_list_button_a11y: string;
    combobox_close_list_button_a11y: string;
    multiselect_instructions: string;
    selected_items: string;
    menu_selection_instructions: string;
    menu_item_expand_arrow: string;
    menu_item_collapse_arrow: string;
    menu_item_collapse_shift_space: string;
    menu_item_shift_space_expand_collapse: string;
    menu_option_list: string;
    menu_item_count: string;
    menu_collapsed: string;
    menu_expanded: string;
    modal_minimize: string;
    modal_maximize: string;
    modal_dock: string;
    modal_close: string;
    selected_date_a11y: string;
    open_calendar_button_a11y: string;
    close_calendar_button_a11y: string;
    open_time_button_a11y: string;
    close_time_button_a11y: string;
    set_current_date_and_time_a11y: string;
    set_current_date_a11y: string;
    clear_current_date_and_time: string;
    clear_current_date: string;
    picker_next_month: string;
    picker_prev_month: string;
    picker_jump_to_month_and_year: string;
    calendar_assist: string;
    calendar: string;
    choose_date: string;
    choose_date_and_time: string;
    choose_week: string;
    boolean_display_true_label: string;
    boolean_display_false_label: string;
    step_changed_to_name: string;
    measured_in: string;
    banner_dismiss_button_label_a11y: string;
    file_upload_text_main: string;
    file_upload_text_one: string;
    file_upload_text_multiple: string;
    my_current_location_button_a11y: string;
    location_input_placeholder: string;
    location_not_found_text: string;
    allow_location_permissions_text: string;
    advanced_search: string;
    select_search_filter: string;
    recent_searches: string;
    clear_search: string;
    search_instructions: string;
    action_from_summarylist_heading: string;
    selected_search_filter: string;
    utilities_label: string;
    utilities_summary: string;
    expand_summary: string;
    collapse_summary: string;
    expand_utilities_panel: string;
    collapse_utilities_panel: string;
    primary_summary_fields: string;
    summary_fields: string;
    accept_suggestion_button_a11y: string;
    reject_suggestion_button_a11y: string;
    suggestion_info: string;
    suggestion_assist: string;
    select_country_code: string;
    enter_a_number: string;
    skip_navigation_menu_label: string;
    tab_error_tooltip: string;
    assignments: string;
    article_list_label_a11y: string;
    article_list_filter_label_a11y: string;
    article_list_header_submit_label: string;
    clear_filters: string;
    suggest_article: string;
    suggested: string;
    followed: string;
    article_label_a11y: string;
    article_related_links: string;
    article_more_like_this: string;
    article_was_this_helpful: string;
    article_thank_you_for_your_feedback: string;
    article_attachments: string;
    likes: string;
    dislikes: string;
    close_preview: string;
    confirmation_whats_next: string;
    confirmation_open_tasks: string;
    add_stakeholders: string;
    edit_stakeholder: string;
    view_stakeholders: string;
    stakeholders_role: string;
    stakeholders: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    edit_tags: string;
    tags: string;
    view_profile: string;
    timeline_toolbar_sort_descending: string;
    timeline_toolbar_sort_ascending: string;
    timeline_toolbar_group_date: string;
    timeline_toolbar_group_monthyear: string;
    timeline_toolbar_group_quarteryear: string;
    timeline_toolbar_group_year: string;
    timeline_toolbar_group_none: string;
    app_announcement_dismiss_button_label_a11y: string;
    app_announcement_details_list_header: string;
    app_announcement_whats_new_button_label: string;
    stages_label: string;
    stages_see_full_lifecycle: string;
    stages_case_lifecycle: string;
    stages_default_case_lifecycle: string;
    stages_stage_label: string;
    predictions: string;
    all_predictions: string;
    learn_more: string;
    tasks_added: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    tasks_removed: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    rte_heading_style: string;
    rte_bold: string;
    rte_italic: string;
    rte_strike_through: string;
    rte_cut: string;
    rte_copy: string;
    rte_paste: string;
    rte_bulleted_list: string;
    rte_numbered_list: string;
    rte_indent: string;
    rte_unindent: string;
    rte_table: string;
    rte_link: string;
    rte_image: string;
    rte_invalid_html: string;
    rte_insert_field: string;
    rte_invalid_url: string;
    rte_change_text_format: string;
    rte_toggle_unordered_list: string;
    rte_indent_selection: string;
    rte_unindent_selection: string;
    rte_toolbar_instructions: string;
    rte_text_formatting_toolbar: string;
    rte_image_description_info: string;
    rte_image_custom_size: string;
    rte_width_in_pixels: string;
    rte_height_in_pixels: string;
    rte_image_settings: string;
    rte_image_key_command: string;
    expand_forwarded_message: string;
    collapse_forwarded_message: string;
    copy_to: string;
    subject_label: string;
    feed_new_post: string;
    feed_new_post_submit: string;
    feed_new_comment: string;
    feed_new_comment_submit: string;
    feed_attach_files: string;
    feed_post_actions_menu: string;
    feed_edit_post: string;
    feed_edit_comment: string;
    feed_context: string;
    feed_liked_by: string;
    feed_likes: string;
    feed_private_post: string;
    feed_public_post: string;
    feed_recipients: string;
    feed_like_label: string;
    feed_unlike_label: string;
    feed_post_likes: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    feed_comment_label: string;
    feed_post_comments: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    feed_attachments_added: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    feed_attachments_removed: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    feed_post_liked: string;
    feed_post_deliked: string;
    feed_post_removed: string;
    feed_comment_removed: string;
    showing_num_of_num: string;
    feed_new_post_added: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    new_item_added: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    loading_post: string;
    mention_type: string;
    scroll_to_latest_message: string;
    scroll_to_unread_messages: string;
    suggested_replies_current_of_total: string;
    suggested_replies: string;
    confidence_percentage: string;
    next_suggested_reply: string;
    prev_suggested_reply: string;
    dismiss_suggested_replies: string;
    message_sent: string;
    temporarilyUnavailable: string;
    available: string;
    unavailable: string;
    authenticated: string;
    not_authenticated: string;
    context: string;
    chat_context: string;
    bot: string;
    is_typing: string;
    chat_message_at_timestamp: string;
    sender_replied_message: string;
    chat_attachments: string;
    chat_links: string;
    chat_message_status: string;
    opened: string;
    undeliverable: string;
    new_message: string;
    attachments: string;
    attachments_x_of_y: string;
    edit_current_template: string;
    send_current_template: string;
    expand_suggested_replies: string;
    collapse_suggested_replies: string;
    replies_count: string;
    no_suggestion: string;
    suggested_reply_copied_to_input_field: string;
    unique_entities: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    more_count: string;
    other_responses: string;
    empty_search: string;
    new_messages: string;
    earlier_transcripts: string;
    send: string;
    compose_label: string;
    save_as_draft: string;
    enter_message: string;
    download: string;
    close_chat_utility: string;
    email_account: string;
    subject: string;
    to: string;
    from: string;
    cc: string;
    bcc: string;
    date: string;
    reply: string;
    reply_all: string;
    response_templates: string;
    forward: string;
    email: string;
    email_message: string;
    email_filters: string;
    unread_email_count: string;
    priority: string;
    inbox: string;
    insert: string;
    email_add_attachment: string;
    email_disable_attachment: string;
    email_remove_attachment: string;
    loading_email: string;
    draft: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    undelivered: string;
    drafts: string;
    unread: string;
    call_panel_heading: string;
    call_panel_make_new_call: string;
    call_panel_agent_status: string;
    call_panel_pause_call_menu_item: string;
    call_panel_resume_call_menu_item: string;
    call_panel_mute_mic_menu_item: string;
    call_panel_unmute_mic_menu_item: string;
    call_panel_call_on_hold: string;
    call_panel_consult_call_menu_item: string;
    call_panel_transfer_call_menu_item: string;
    call_panel_menu_item_call_only: string;
    call_panel_menu_item_call_with_interaction: string;
    call_panel_send_dtmf_menu_item: string;
    call_panel_conference_call_menu_item: string;
    call_panel_hangup_call_menu_item: string;
    call_panel_contacts_add_to_favorites: string;
    call_panel_contacts_remove_from_favorites: string;
    call_panel_contacts_call_button_label: string;
    call_panel_contacts_list_heading: string;
    call_panel_contacts_favorites_heading: string;
    call_panel_contacts_dial_pad_heading: string;
    call_panel_new_call_heading: string;
    call_panel_interaction_transfer_heading: string;
    call_panel_interaction_transfer_complete_button_label: string;
    call_panel_incoming_call: string;
    call_panel_answer_call: string;
    call_panel_conference_heading: string;
    call_panel_conference_call_and_interactions_heading: string;
    call_panel_consult_heading: string;
    call_panel_consult_call_and_interactions_heading: string;
    call_panel_transfer_heading: string;
    call_panel_transfer_call_and_interactions_heading: string;
    call_panel_conference_submit: string;
    call_panel_transfer_submit: string;
    call_panel_consult_submit: string;
    call_panel_handover_reason_label: string;
    call_panel_handover_to_label: string;
    call_panel_handover_comments_label: string;
    call_panel_handover_call_option_label: string;
    call_panel_handover_call_only_option: string;
    call_panel_handover_call_and_interaction_option: string;
    call_panel_handover_interaction_label: string;
    call_panel_merge_call_menu_item: string;
    call_panel_handoff_call_menu_item: string;
    call_panel_send_dtmf_heading: string;
    call_panel_expand: string;
    call_panel_collapse: string;
    call_panel_nav_label: string;
    dial_pad_keyboard: string;
    dial_pad_phone_number_input_label: string;
    dial_pad_call_button_label: string;
    interaction_time_expired: string;
    interaction_time_remaining: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    interaction_shortcut_goto_info: string;
    interaction_shortcut_dismiss_info: string;
    shortcut_GoToIncomingInteraction: string;
    shortcut_DismissIncomingInteraction: string;
    select_multiple: string;
    dismiss_case: string;
    task_manager_add_case: string;
    task_manager_wrap_up: string;
    task_manager_cases: string;
    add_cases: string;
    open_cases: string;
    resolved_cases: string;
    no_picker_items: string;
    selected_picker_items: string;
    no_selected_picker_items: string;
    search_picker_items: string;
    case_picker_close: string;
    case_manager_overflow_menu: string;
    continue_label: string;
    continue_label_a11y: string;
    review_label: string;
    review_label_a11y: string;
    dismiss_label: string;
    dismiss_label_a11y: string;
    task_manager_picker_add_label: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    show_detected_entities: string;
    nothing_detected_yet: string;
    ai_detected_entities: string;
    close_conversation: string;
    shortcut_AddTask: string;
    shortcut_WrapUp: string;
    live_transcript: string;
    open_live_transcript: string;
    close_live_transcript: string;
    download_live_transcript: string;
    verbatim: string;
    dialog: string;
    sentiment_positive: string;
    sentiment_negative: string;
    sentiment_neutral: string;
    no_recommendations: string;
    condition_builder_advanced_condition_label: string;
    condition_builder_advanced_condition_placeholder: string;
    condition_builder_advanced_mode_button_label: string;
    condition_builder_basic_mode_button_label: string;
    condition_builder_advanced_mode_button_aria_label: string;
    condition_builder_basic_mode_button_aria_label: string;
    condition_builder_mode_switch: string;
    condition_builder_advanced_condition_tooltip: string;
    condition_builder_condition_select_info_default: string;
    condition_builder_possible_values_label: string;
    condition_builder_possible_values_error: string;
    condition_builder_multi_selection_text_default: string;
    condition_builder_single_selection_text_default: string;
    condition_builder_switch_banner_text: string;
    condition_builder_confirm_button_label: string;
    condition_builder_empty_selection_error_text: string;
    condition_builder_empty_value_text: string;
    condition_builder_compare_with: string;
    condition_builder_compare_with_another_field: string;
    condition_builder_compare_with_parameter: string;
    condition_builder_compare_with_relative_date: string;
    condition_builder_compare_with_text_value: string;
    condition_builder_compare_with_numeric_value: string;
    condition_builder_compare_with_date_value: string;
    condition_builder_compare_with_time_value: string;
    condition_builder_compare_with_numeric_range_value: string;
    condition_builder_compare_with_date_range_value: string;
    condition_builder_compare_with_time_range_value: string;
    condition_builder_invalid_time_period: string;
    condition_builder_invalid_relative_date: string;
    condition_builder_invalid_value: string;
    condition_builder_invalid_values: string;
    condition_builder_invalid_time: string;
    condition_builder_invalid_date: string;
    condition_builder_invalid_number: string;
    condition_builder_invalid_date_range: string;
    condition_builder_invalid_time_range: string;
    condition_builder_invalid_number_range: string;
    condition_builder_select_placeholder: string;
    condition_builder_hour_number_of_day: string;
    condition_builder_rhs_with_time_only_input: string;
    condition_builder_rhs_with_date_only_input: string;
    condition_builder_rhs_with_date_time_input: string;
    condition_builder_rhs_with_csv_input: string;
    condition_builder_no_selection: string;
    condition_builder_enter_csv_placeholder: string;
    condition_builder_day_number_of_month: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_year: string;
    condition_builder_quarter: string;
    condition_builder_month: string;
    condition_builder_week: string;
    condition_builder_day: string;
    condition_builder_hour: string;
    condition_builder_minute: string;
    condition_builder_second: string;
    condition_builder_date_function_hour_of_day: string;
    condition_builder_date_function_month_of_year: string;
    condition_builder_date_function_day_of_month: string;
    condition_builder_date_function_day_of_week: string;
    condition_builder_date_part: string;
    condition_builder_current_second: string;
    condition_builder_current_minute: string;
    condition_builder_current_hour: string;
    condition_builder_current_day: string;
    condition_builder_current_week: string;
    condition_builder_current_month: string;
    condition_builder_current_quarter: string;
    condition_builder_current_year: string;
    condition_builder_last_X_seconds: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_last_X_minutes: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_last_X_hours: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_last_X_days: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_last_X_weeks: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_last_X_months: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_last_X_quarters: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_last_X_years: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_next_X_seconds: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_next_X_minutes: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_next_X_hours: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_next_X_days: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_next_X_weeks: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_next_X_months: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_next_X_quarters: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_next_X_years: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_previous_X_seconds: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_previous_X_minutes: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_previous_X_hours: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_previous_X_days: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_previous_X_weeks: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_previous_X_months: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_previous_X_quarters: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_previous_X_years: {
        zero: string;
        one: string;
        two: string;
        few: string;
        many: string;
        other: string;
    };
    condition_builder_time_period_label: string;
    condition_builder_time_period_last: string;
    condition_builder_time_period_current: string;
    condition_builder_time_period_previous: string;
    condition_builder_time_period_next: string;
    condition_builder_value_label: string;
    condition_builder_field_label: string;
    condition_builder_comparator_label: string;
    promoted_filters_clear_all_button_label: string;
    dev_mode: string;
    expand_navigation: string;
    collapse_navigation: string;
    switch_to: string;
    expand_search: string;
    collapse_search: string;
    branch_with_no_changes: string;
    branch_with_no_potential_conflicts: string;
    branch_with_potential_conflicts: string;
    parallel: string;
    stage: string;
    alternate_stage: string;
    add_stage: string;
    add_process: string;
    add_alternate_stage: string;
    stage_name: string;
    process_name: string;
    step_name: string;
    item_error: string;
    step_name_error: string;
    stage_name_error: string;
    process_name_error: string;
    add_node: string;
    search_nodes: string;
    delete_step: string;
    delete_decision_step: string;
    delete_step_title: string;
    delete_step_outcome: string;
    delete_all_outcomes: string;
    delete_step_error: string;
    delete_referenced_step_error: string;
    delete_connector_error: string;
    zoom_level: string;
    close_configuration_panel: string;
    clear_object_summary: string;
    create_in_new_tab: string;
    expression_editor: string;
    output_parameters: string;
    input_parameters: string;
    no_app_build: string;
    build_app: string;
    build_in_progress: string;
    rebuild_app: string;
    download_app: string;
    qr_code_not_available: string;
    qr_code: string;
    app_header_notifications_empty: string;
    app_header_notifications_loading: string;
    open_app_header_notifications: string;
    open_app_header_drawer: string;
    preview_frame: string;
    open_in_new_window_text: string;
    more_options: string;
    dev_tools: string;
    active_session_info: string;
    continue_debugging: string;
}>>;
export declare const direction: (localeTag: string) => 'ltr' | 'rtl';
//# sourceMappingURL=i18n.d.ts.map