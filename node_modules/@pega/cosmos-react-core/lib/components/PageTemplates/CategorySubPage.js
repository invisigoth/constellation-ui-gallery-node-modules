...
const NavItem = ({ item: { href, text, active, ...restProps } }) => {
    const isAnchor = !!href;
    return (_jsx(StyledNavItem, { as: 'li', selected: !!active, tabType: 'horizontal', "aria-current": active ? 'page' : undefined, children: _jsx(Button, { ...restProps, href: isAnchor ? href : undefined, variant: 'text', children: text }) }));
};
const NavGroup = ({ item }) => {
...
const CategorySubPage = forwardRef(function CategorySubPage({ viewLoading = false, category, categoryIcon, navItemGroups, pageTitle, pageActions, region, maxActions = 3, ...restProps }, ref) {
    ...
    return (_jsx(StyledTemplate, { children: _jsxs(Flex, { ...restProps, ref: templateRef, as: StyledTemplateRow, container: { alignItems: 'stretch', wrap: 'nowrap' }, children: [_jsxs(StyledMenuColumn, { children: [_jsx(StyledSkipNavigation, { onClick: () => {
                                if (articleRef.current)
                                    ...
                                    const key = 'title' in navItem ? navItem.title : `navItem${index}`;
