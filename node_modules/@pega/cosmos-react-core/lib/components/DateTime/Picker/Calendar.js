...
import { StyledColumnLabel, StyledPick, StyledDay, StyledCalendar, StyledWeekGrid, StyledWeekLabel, StyledWeekNumber } from './Calendar.styles';
const ariaOpts = { day: 'numeric', month: 'long', year: 'numeric' };
const Calendar = forwardRef(function Calendar({ testId, selected, current = selected ? new Date(selected.getTime()) : getAdjustedUTCDateTime(), selectedRange, showHighlight, mode = 'date', showWeekNumber, calendarType: calendar = CALENDAR_TYPES.ISO_8601, onSelect, onFocus, disabled, min: minProp, max: maxProp }, ref) {
    ...
    const weeks = getWeeksOfMonth(current, calendarType).map(weekNo => {
        return (_jsxs(_Fragment, { children: [_jsx(Text, { as: StyledWeekNumber, "aria-hidden": 'true', children: weekNo.toString() }, weekNo), _jsx(VisuallyHiddenText, { children: `${displayNames.of('weekOfYear')} ${weekNo.toString()} ${current
                        .getUTCFullYear()
                        .toString()}` })] }));
    });
    ...
    const weekLabel = mode === 'date' ? (_jsxs(_Fragment, { children: [_jsx(Text, { as: StyledWeekLabel, forwardedAs: 'abbr', title: cap(displayNames.of('weekOfYear')), "aria-hidden": 'true', children: cap(displayNamesShort.of('weekOfYear')) }), _jsx(VisuallyHiddenText, { children: displayNames.of('weekOfYear') })] })) : (_jsx(Text, { as: StyledWeekLabel, children: cap(displayNames.of('weekOfYear')) }));
    const headerLabels = names.map(([name, abbr]) => (_jsxs("th", { role: 'columnheader', children: [_jsx(Text, { as: StyledColumnLabel, forwardedAs: 'abbr', title: name, "aria-hidden": 'true', children: abbr }, name), _jsx(VisuallyHiddenText, { children: name })] }, name)));
    ...
    return (_jsxs(_Fragment, { children: [mode === 'week' && weeksGrid, _jsxs(StyledCalendar, { "data-testid": testId, ref: mode === 'date' ? containerRef : undefined, "aria-label": current.toLocaleString(locale, {
                    month: 'long',
                    year: 'numeric',
                    timeZone: 'UTC'
                }), onKeyDown: onKeyDown, "aria-hidden": mode === 'week' ? true : undefined, role: 'grid', children: [_jsx("thead", { children: _jsxs("tr", { children: [showWeekLabel && _jsx("th", { children: weekLabel }), headerLabels] }) }), _jsx("tbody", { children: cells })] }), _jsx(VisuallyHiddenText, { "aria-live": 'polite', role: 'alert', children: t('calendar_assist') })] }));
});
