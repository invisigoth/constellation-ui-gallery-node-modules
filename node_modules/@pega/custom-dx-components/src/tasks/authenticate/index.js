...
import {
  TOKEN_PATH,
  INF_TOKEN_URL_V1,
  INF_AUTHORIZE_URL_V1,
  LP_TOKEN_URL,
  LP_AUTHORIZE_URL
} from '../../constants.js';
import { PegaAuth } from '@pega/auth';

import { getAuthQuestions } from './helper.js';

export const TASKS_CONFIG_JSON_FILENAME = 'tasks.config.json';
...
const generateAccessToken = async config => {
  ...
  // Try to gracefully deal with missing trailing slashes for server (and later conditionally for redirectUri)
  const server = config.server.endsWith('/') ? config.server : `${config.server}/`;

  return new Promise((resolve, reject) => {
    const sTokenUri = `${server}${serverType === 'infinity' ? INF_TOKEN_URL_V1 : LP_TOKEN_URL}`;
    const sAuthorizeUri = `${server}${
      serverType === 'infinity' ? INF_AUTHORIZE_URL_V1 : LP_AUTHORIZE_URL
    }`;
    // console.log(`grantType: ${config.grantType}`);
    const browserAuthSuccessMarkup = `<div style="width: 300px; padding: 10px;">
    <svg xmlns="http://www. w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 311 68" version="1.1">
    <title>Logo</title>
    <defs/>
    ...
    <div style="font-size: 2em; font-family: Arial; padding: 10px">
    Authentication is complete!!
    </div>
    <div style="font-size: 13m; font-family: Arial; padding: 10px">
    This browser tab or window may be closed.
    </div>
    ...
    const authCodeConfig =
      grantType === 'authCode'
        ? {
            authorizeUri: sAuthorizeUri,
            redirectUri: config.redirectUri.endsWith('/')
              ? config.redirectUri
              : `${config.redirectUri}/`,
            cert: config.cert,
            key: config.key,
            winTitle: 'DX Component Dev',
            winBodyHtml: browserAuthSuccessMarkup
          }
        : {};

    // Build auth config structure
    const authConfig = {
      serverType,
      authService,
      grantType,
    ...
    if (legacyTLS) {
      authConfig.legacyTLS = true;
    }
    if (serverType === 'infinity') {
      if (grantType !== 'clientCreds' && user && password) {
        authConfig.userIdentifier = user;
        authConfig.password = Buffer.from(password).toString('base64');
      }
    }
    if (serverType === 'launchpad') {
      if (isolationId) {
        authConfig.isolationId = isolationId;
      }
      authConfig.noPKCE = true;
    }
    ...
    try {
      ...
      if (grantType === 'authCode') {
        console.log(
          `${chalk.green(
            'Awaiting authorization to complete (Ctrl-C to exit).  You may need to enter credentials within your default browser.'
          )}`
        );
      }
      ...
      auth
        ...
        .then(token => {
          if (token?.token_type) {
            fs.writeFile(TOKEN_PATH, JSON.stringify(token), err => {
              if (err) {
                console.error(err);
                return reject(authSuccess);
              }
              console.log(`${chalk.bold.green('Authenticated successfully !!')}`);
              authSuccess = true;
              return resolve(authSuccess);
            });
          } else {
            console.log(
              `${chalk.bold.red(
                `Authentication failed:${JSON.stringify(token.errors ? token.errors : token)}`
              )}`
            );
            return reject(authSuccess);
          }
        ...
        .catch(e => {
          console.log(`${chalk.bold.red(`Authentication failed:${e}`)}`);
          return reject(authSuccess);
        });
    } catch (e) {
      console.log(`Authentication failed: Problem initializing OAuth library`);
      return reject(authSuccess);
    }
  });
};
...
export default async options => {
  ...
  const cfgData = fs.readFileSync(pegaConfigJsonPath, { encoding: 'utf8' });
  let data = null;
  if (cfgData) {
    try {
      data = JSON.parse(cfgData);
    } catch (e) {
      console.log(`${chalk.red('Invalid JSON within tasks.config.json')}`);
      process.exit(1);
    }
  }

  const serverConfig = data['server-config'];
  const isPwdNonEmpty = !!serverConfig.password;

  // console.log(`serverConfig: ${JSON.stringify(serverConfig)}`);
  const questions = await getAuthQuestions(serverConfig);
  // console.log(`questions: ${JSON.stringify(questions)}`);
  const answers = await inquirer.prompt(questions);
  const aAsked = answers.aAsked;
  delete answers.aAsked;
  const aProps = [
    'serverType',
    'server',
    'isolationId',
  ...
  const isLaunchPad = answers.serverType === 'launchpad';
  let bWriteUpdate = false;
  for (let i = 0; i < aProps.length; i += 1) {
    const prop = aProps[i];
    const configValue = serverConfig[prop];
    const answerValue = answers[prop];
    // Treat an empty serverConfig property value as same as an undefined answer response
    if (configValue !== answerValue && !(configValue === '' && answerValue === undefined)) {
      // console.log(`property ${prop} changed from ${configValue} to ${answerValue}`);
      serverConfig[prop] = answerValue;
      if (prop !== 'password' || isPwdNonEmpty) {
        bWriteUpdate = true;
      }
    }
  }
  // Update the askedOnce array based on askOnce criteria
  const aAskOnce = serverConfig.questions_askOnce ? serverConfig.questions_askOnce.split(',') : [];
  const aAskedOnce = serverConfig.questions_askedOnce
    ? serverConfig.questions_askedOnce.split(',')
    : [];
  let bAskedOnceUpdated = false;
  for (let j = 0; j < aAsked.length; j += 1) {
    const prop = aAsked[j];
    if (aAskOnce.includes(prop) && !aAskedOnce.includes(prop)) {
      // Add entries we want to only ask once to the asked list if appropriate
      aAskedOnce.push(prop);
      bAskedOnceUpdated = true;
    }
  }
  // If askedOnce array was updated we need to update value in config
  if (bAskedOnceUpdated) {
    serverConfig.questions_askedOnce = aAskedOnce.join(',');
    bWriteUpdate = true;
  }

  try {
    // console.log(`serverConfig: ${JSON.stringify(serverConfig)}`);
    authOK = await generateAccessToken(serverConfig);
  } catch (e) {
    console.log(`${chalk.bold.red(`Authentication failed:${JSON.stringify(e)}`)}`);
  }

  if (authOK & bWriteUpdate) {
    // Don't write password value (unless it already had a value)
    if (!isPwdNonEmpty) {
      serverConfig.password = '';
    }
    console.log(`${chalk.green('Writing updated config...')}`);
    let updCfg = null;
    try {
      updCfg = JSON.stringify(data, null, 2);
      // console.log(`updated config: ${updCfg}`);
      fs.writeFileSync(pegaConfigJsonPath, updCfg, err => {
        if (err) throw err;
      });
    } catch (e) {}
  }
