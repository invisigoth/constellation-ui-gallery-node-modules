...
describe('create build and delete local', () => {
  ...
  it('npm run buildComponent', async () => {
    const fileName = 'MyTestText';
    const newFileName = `Pega_DXIL_${fileName}`;

    const script = `npm run buildComponent ${newFileName} "" N --prefix ../../..`;
    const { stdout, stderr } = await exec(script);
    // console.log(stdout);
    expect(stdout).not.toBeNull();
    expect(stderr).not.toBeNull();
  ...
  it('npm run delete Local', async () => {
    const fileName = 'MyTestText';
    const newFileName = `Pega_DXIL_${fileName}`;
    const script = `npm run delete Local ${newFileName} Y --prefix ../../..`;
