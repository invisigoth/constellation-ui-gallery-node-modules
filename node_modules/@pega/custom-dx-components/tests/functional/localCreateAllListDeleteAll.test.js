...
describe('createAll list and deleteAll local', () => {
  ...
  it('npm run createAll Local', async () => {
    const script = `npm run createAll "My" 0.0.1 DXIL "" Pega --prefix ../../..`;
  ...
  it('npm run list Local', async () => {
    const script = 'npm run list Local --prefix ../../..';
    const { stdout, stderr } = await exec(script);
  ...
  it('compare created to default', async () => {
    ...
    const tasksConfigJSON = JSON.parse(tasksConfigData);
    ...
    compList.forEach(component => {
      ...
      const assetDir = path.resolve(`tests/assets/components/createAll/${serverDir}/${component}`);
