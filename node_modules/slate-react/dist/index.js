...
var useChildren = function useChildren(props) {
  ...
  var _loop = function _loop(i) {
    ...
    var ds = decorations.reduce(function (acc, dec) {
      var intersection = slate.Range.intersection(dec, range);
...
var Editable$1 = function Editable(props) {
  ...
  var decorations = _toConsumableArray(slate.Node.nodes(editor)).flatMap(function (_ref) {
    ...
    onClick: React.useCallback(function (event) {
      if (hasTarget(editor, event.target) && !isEventHandled(event, attributes.onClick) && isDOMNode(event.target)) {
        ...
        if (event.detail === TRIPLE_CLICK) {
          var _start = slate.Editor.start(editor, [path[0]]);

          var end = slate.Editor.end(editor, [path[0]]);
          var range = slate.Editor.range(editor, _start, end);
          slate.Transforms.select(editor, range);
          return;
        }
        ...
        if (slate.Editor.hasPath(editor, path)) {
          ...
          if (lookupNode === node) {
            var _start2 = slate.Editor.start(editor, path);

            var _end = slate.Editor.end(editor, path);
            ...
            if (startVoid && endVoid && slate.Path.equals(startVoid[1], endVoid[1])) {
              var _range = slate.Editor.range(editor, _start2);
...
var DefaultPlaceholder = function DefaultPlaceholder(_ref3) {
